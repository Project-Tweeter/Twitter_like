...
class User {
        ...
        static createUser(nom, prenom, email, birthday, password, username, link, callback){
            ...
            db.query('INSERT INTO users SET nom = ?, prenom = ?, email = ?, birthday = ?,  password = ?, username = ?, link = ?', [nom,prenom,email,birthday,password,username,link], (error, result) => { 
        ...
        static findUser(username, callback) {
          db.query("SELECT * FROM users WHERE username = ?", [username], (error, user) =>{
            if (error) throw error
            callback(error, user);
          });
